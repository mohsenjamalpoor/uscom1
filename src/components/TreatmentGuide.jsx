...
export default function TreatmentGuide({ parameters, patient, darkMode }) {
  const { status, parameters: hemodynamicParams } = useHemodynamic(parameters);
  ...
  const recommendations = {
    fluids: {
      ...
      getRecommendation: () => {
        ...
        if (
          status.type === "Hyperdynamic" &&
          shouldShowRecommendation([svr]) &&
          svr < 800
        ) {
          return {
    ...
    noradrenaline: {
      ...
      getRecommendation: () => {
        if (
          status.type === "Hyperdynamic" &&
          shouldShowRecommendation([svr, map]) &&
          svr < 800 &&
          map < 65
        ) {
        ...
        if (
          status.type === "Hypodynamic" &&
          shouldShowRecommendation([smii, svr]) &&
          smii > 1.2 &&
          svr > 1600
        ) {
    ...
    dopamine: {
      ...
      getRecommendation: () => {
        if (
          status.type === "Hypodynamic" &&
          shouldShowRecommendation([smii, hr]) &&
          smii < 1.0 &&
          hr < 60
        ) {
